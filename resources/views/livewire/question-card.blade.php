            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 min-h-[500px]">
                ...
                <div class="p-8 sm:p-12 bg-gradient-to-br from-primary-50 to-indigo-50 flex flex-col">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <div class="inline-flex items-center px-3 py-1.5 bg-white/80 backdrop-blur-sm rounded-full">
                            <svg class="w-4 h-4 text-primary-600 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M2 6a2 2 0 012-2h6a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V6zM14.553 7.106A1 1 0 0014 8v4a1 1 0 00.553.894l2 1A1 1 0 0018 13V7a1 1 0 00-1.447-.894l-2 1z"/>
                            </svg>
                            <span class="text-xs font-semibold text-primary-700">{{ $question->categoria }}</span>
                        </div>
                        <span class="text-xs text-gray-500 font-medium">#{{ $question->numero }}</span>
                    </div>

                    <!-- Question Text -->
                    <div class="flex-1 flex items-center">
                        <p class="text-lg leading-relaxed text-gray-900 whitespace-pre-line">{{ $question->enunciado }}</p>
                    </div>
                </div>

                <!-- Right Panel: Options & Explanation -->
                <div class="p-8 sm:p-12 flex flex-col">
                        ...
                        @foreach((count($shuffledOptions) > 0 ? $shuffledOptions : ['A', 'B', 'C', 'D']) as $option)
                            ...
                            >
                                <div class="flex items-start">
                                    <!-- Option Letter -->
                                    <div class="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg font-bold mr-3 text-sm transition-colors
                                    ...
                                    <!-- Option Text -->
                                    <span class="flex-1 pt-0.5 text-sm text-gray-900 leading-relaxed">
                                        {{ $question->{'opcao_' . strtolower($option)} }}
                                    </span>

                                    <!-- Check/Cross Icon -->
                                    @if($answered && $option === $question->resposta_correta)
                                        <svg class="w-5 h-5 text-emerald-600 ml-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($answered)
                        <div class="space-y-4">
                            <!-- Feedback Badge -->
                            <div class="flex items-center gap-3 p-4 rounded-xl {{ $wasCorrect ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200' }}">
                                <div class="text-2xl">
